 ...
 */function ut(t){return pt({...t,state:!0,attribute:!1})}class mt extends at{}t([pt({attribute:!1})],mt.prototype,"hass",void 0);const gt="chores-manager-daily-card",_t="chores-manager-overview-card",ft="chores-manager-correction-card",yt="chores-manager-history-card",wt=new Set(["unknown","unavailable"]);function bt(t,e){return t.attributes[e]}function vt(t,e){const i=Number(bt(t,e));return Number.isFinite(i)?i:0}function $t(t,e,i){return(i?t.states[i]:void 0)??Object.entries(t.states).find(([t,i])=>t.startsWith("sensor.")&&bt(i,"child_id")===e)?.[1]}function xt(t){const e=new Map;for(const i of Object.values(t.states)){const t=bt(i,"child_id");if(!t)continue;const s=bt(i,"kid_name")??bt(i,"child_name");s?.trim()?e.set(t,s):e.has(t)||e.set(t,t)}return[...e.entries()].map(([t,e])=>({id:t,name:e})).sort((t,e)=>t.name.localeCompare(e.name))}function kt(t,e){const i=e.child_entity?t.states[e.child_entity]:void 0;return e.child_id??(i?bt(i,"child_id"):void 0)}function Ct(t,e){return xt(t).find(t=>t.id===e)?.name}function At(t,e){return Object.entries(t.states).filter(([t,i])=>t.startsWith("switch.")&&!wt.has(i.state)&&bt(i,"child_id")===e&&"string"==typeof bt(i,"assignment_id")).map(([t,i])=>({assignmentId:bt(i,"assignment_id")??t,entityId:t,childId:e,title:bt(i,"title")??bt(i,"friendly_name")??t,category:bt(i,"category")??"Other",points:vt(i,"points"),icon:bt(i,"icon")??"mdi:checkbox-marked-circle-outline",sortOrder:vt(i,"sort_order"),completed:"on"===i.state})).sort((t,e)=>t.category.localeCompare(e.category)||t.sortOrder-e.sortOrder||t.title.localeCompare(e.title))}function Pt(t,e,i){const s=$t(t,e,i);if(!s||wt.has(s.state))return;const o=Number(s.state);return Number.isFinite(o)?o:void 0}function St(t,e,i){const s=$t(t,e,i),o=s?bt(s,"week_start"):void 0;return"string"==typeof o?o:void 0}function Et(t,e){const i=e?t.states[e]:void 0;return i?bt(i,"entity_picture"):void 0}const jt={en:{chores:"Chores",completed:"Goal reached",no_chores:"No available chores.",points:"points",remaining:"remaining for",rewards:"Points & rewards",how_points_work:"How points work",reward_levels:"Rewards",daily:"Chores",history:"History",correction:"Correction",previous_week:"Previous week",adjust_points:"Adjust points",adjustment_amount:"Amount",adjustment_reason:"Reason (optional)",add_points:"Add points",subtract_points:"Subtract points",adjustment_error:"The point adjustment could not be saved.",weekly_points_error:"Weekly totals could not be loaded.",correction_error:"Correction data could not be loaded.",correction_child_not_found:"This child no longer exists. Select an available child in the card editor.",previous_date:"Previous date",next_date:"Next date",choose_date:"Choose date",close:"Close",back:"Back",correct_chores:"Correct chores",adjust:"Adjust",show:"View",add_completion:"Add completion",remove_completion:"Remove completion",weekly_chores:"Weekly chores",history_error:"Chore history could not be loaded.",history_empty:"No chores logged this week.",total:"Total"},sv:{chores:"Sysslor",completed:"Målet är uppnått",no_chores:"Inga tillgängliga sysslor.",points:"poäng",remaining:"kvar till",rewards:"Poäng & belöningar",how_points_work:"Så fungerar poängen",reward_levels:"Belöningar",daily:"Sysslor",history:"Historik",correction:"Korrigering",previous_week:"Förra veckan",adjust_points:"Justera poäng",adjustment_amount:"Antal",adjustment_reason:"Orsak (valfri)",add_points:"Lägg till poäng",subtract_points:"Dra av poäng",adjustment_error:"Poängjusteringen kunde inte sparas.",weekly_points_error:"Veckopoängen kunde inte hämtas.",correction_error:"Korrigeringsdata kunde inte hämtas.",correction_child_not_found:"Barnet finns inte längre. Välj ett tillgängligt barn i kortets redigerare.",previous_date:"Föregående datum",next_date:"Nästa datum",choose_date:"Välj datum",close:"Stäng",back:"Tillbaka",correct_chores:"Korrigera sysslor",adjust:"Justera",show:"Visa",add_completion:"Lägg till genomförd syssla",remove_completion:"Ta bort genomförd syssla",weekly_chores:"Veckans sysslor",history_error:"Historiken kunde inte hämtas.",history_empty:"Inga sysslor registrerade den här veckan.",total:"Totalt"}};function Mt(t,e){return"en"===t||"sv"===t?t:e?.language?.toLowerCase().startsWith("sv")?"sv":"en"}function zt(t,e,i){return jt[Mt(e,i)][t]}let Dt=class extends mt{constructor(){super(...arguments),this.pendingCompletions=new Map}static getConfigElement(){return document.createElement("chores-manager-daily-card-editor")}static getStubConfig(){return{child_id:"kid_1",locale:"auto"}}setConfig(t){if(!t?.child_id?.trim()&&!t?.child_entity?.trim())throw new Error("child_id or child_entity is required");this.config={locale:"auto",show_border:!0,show_header:!0,show_person:!0,show_points:!0,...t},this.requestUpdate()}getCardSize(){return 4}render(){if(!this.hass||!this.config)return L;const t=kt(this.hass,this.config);if(!t)return L;const e=At(this.hass,t),i=function(t){return t.reduce((t,e)=>{const i=t.get(e.category)??[];return i.push(e),t.set(e.category,i),t},new Map)}(e.map(t=>({...t,completed:this.pendingCompletions.get(t.entityId)??t.completed}))),s=Pt(this.hass,t,this.config.weekly_points_entity??this.config.child_entity),o=void 0===s?void 0:s+e.reduce((t,e)=>{const i=this.pendingCompletions.get(e.entityId)??e.completed;return i===e.completed?t:t+(i?e.points:-e.points)},0),n=Et(this.hass,this.config.person_entity),r=this.config.name??this.config.title??Ct(this.hass,t)??zt("chores",this.config.locale,this.hass);return V`
      <ha-card class=${!1===this.config.show_border?"borderless":""}>
        ${!1!==this.config.show_header?V`
              <header>
                ...
                <div>
                  <h1>${r}</h1>
                  ${!1!==this.config.show_points&&void 0!==o?V`<p data-weekly-points>${o} ${zt("points",this.config.locale,this.hass)}</p>`:L}
                </div>
              </header>
            `:L}
        ${this.error?V`<p class="error" role="alert">${this.error}</p>`:L}
        ${0===i.size?V`<p class="empty">${zt("no_chores",this.config.locale,this.hass)}</p>`:[...i.entries()].map(([t,e])=>this.renderGroup(t,e))}
      </ha-card>
  ...
  `}};t([ut()],Dt.prototype,"error",void 0),t([ut()],Dt.prototype,"pendingCompletions",void 0),Dt=t([lt(gt)],Dt);let Tt=class extends mt{constructor(){super(...arguments),this.dateInputReady=Boolean(customElements.get("ha-date-input")),this.goBack=()=>history.back(),this.close=()=>{location.hash?location.hash="":history.back()}}static getConfigElement(){return document.createElement("chores-manager-correction-card-editor")}static getStubConfig(t){return{child_id:t?xt(t)[0]?.id??"kid_1":"kid_1",locale:"auto",show_border:!0,show_header:!0}}connectedCallback(){super.connectedCallback(),this.loadDateInput()}setConfig(t){if(!t?.child_id?.trim())throw new Error("child_id is required");this.config={locale:"auto",show_border:!0,show_header:!0,...t},this.load(),this.requestUpdate()}willUpdate(){this.load()}getCardSize(){return 9}render(){if(!this.hass||!this.config)return L;const t=this.inventory?.children.find(t=>t.child_id===this.config?.child_id),e=this.config.name??t?.name??this.config.child_id,i=Et(this.hass,this.config.person_entity);return V`
      <ha-card class=${!1===this.config.show_border?"borderless":""}>
        ${!1!==this.config.show_header?V`
              <header>
                ${i?V`<img class="portrait" src=${i} alt="" />`:V`<ha-icon class="portrait-icon" icon="mdi:account-circle"></ha-icon>`}
                <strong class="title">${zt("correct_chores",this.config.locale,this.hass)} - ${e}</strong>
                <span class="header-points">${this.weeklyPoints?.current_week.points??0}p</span>
                <button class="header-button" aria-label=${zt("back",this.config.locale,this.hass)} @click=${this.goBack}>
                  <ha-icon icon="mdi:arrow-left"></ha-icon>
                </button>
                <button class="header-button" aria-label=${zt("close",this.config.locale,this.hass)} @click=${this.close}>
                  <ha-icon icon="mdi:close"></ha-icon>
                </button>
              </header>
            `:L}
        ${this.error?V`<p class="error" role="alert">${zt("child_not_found"===this.error?"correction_child_not_found":"correction_error",this.config.locale,this.hass)}</p>`:this.history&&this.inventory&&this.selectedDate?V`
                ${this.renderDateNavigation()}
                <div class="groups">${this.renderGroups()}</div>
              `:L}
      </ha-card>
    `}renderDateNavigation(){const t=Mt(this.config?.locale,this.hass),e=new Date(`${this.selectedDate}T12:00:00`),i=new Intl.DateTimeFormat("sv"===t?"sv-SE":"en-US",{weekday:"short",day:"numeric",month:"short"}).format(e),s=this.selectedDate===this.history?.window.start,o=this.selectedDate===this.history?.window.end;return V`
      <section class="date-navigation">
        <h1>${i}</h1>
        <div class="date-actions">
          <button ?disabled=${s} aria-label=${zt("previous_date",this.config?.locale,this.hass)} @click=${()=>this.shiftDate(-1)}>
            <ha-icon icon="mdi:chevron-left"></ha-icon>
          </button>
          <button ?disabled=${o} aria-label=${zt("next_date",this.config?.locale,this.hass)} @click=${()=>this.shiftDate(1)}>
          ...
          >
            ...
            <ha-date-input
              .label=${zt("choose_date",this.config?.locale,this.hass)}
    ...
    `}renderGroups(){const t=new Map;for(const e of this.rows()){const i=t.get(e.chore.category)??[];i.push(e),t.set(e.chore.category,i)}return[...t].map(([t,e])=>V`
      ...
      `)}renderRow(t){const e=this.pendingAssignment===t.assignment.assignment_id,i=t.completed?zt("remove_completion",this.config?.locale,this.hass):zt("add_completion",this.config?.locale,this.hass);return V`
      <div class="chore-row">
  ...
  `}};t([ut()],Tt.prototype,"inventory",void 0),t([ut()],Tt.prototype,"history",void 0),t([ut()],Tt.prototype,"weeklyPoints",void 0),t([ut()],Tt.prototype,"selectedDate",void 0),t([ut()],Tt.prototype,"pendingAssignment",void 0),t([ut()],Tt.prototype,"error",void 0),t([ut()],Tt.prototype,"dateInputReady",void 0),Tt=t([lt(ft)],Tt);const Ut={entity:{filter:[{domain:"sensor",integration:"chores_manager"}]}},It={ui_action:{actions:["more-info","navigate","url","toggle","perform-action","call-service","assist","none"],default_action:"none"}},Ot=[{label:"Chores",icon:"mdi:format-list-checks",color:"#00bcd4"},{label:"History",icon:"mdi:trophy-outline",color:"#ffc107"},{label:"Correction",icon:"mdi:wrench-cog",color:"#9c27b0"}];function Rt(t){const{visibility:e,...i}=t;return{...i,visibility_mode:e?.mode??"all",visibility_users:e?.users??[]}}function Ht(t){const e=[t.daily_action,t.history_action,t.correction_action];return Ot.flatMap((t,i)=>{const s=e[i];return s&&"none"!==s.action?[{...t,tap_action:s}]:[]})}function qt(t,e){if("correction"===e)return{locale:"auto",show_border:!0,show_header:!0,...t};if("history"===e)return{locale:"auto",show_border:!0,show_header:!0,show_person:!0,show_points:!0,...t};const i={locale:"auto",show_header:!0,show_name:!0,show_person:!0,show_points:!0,show_border:!0,person_position:"center",person_size:"medium"};if("overview"===e){const e=t;return{...i,show_previous_week:!0,show_adjustments:!0,...t,...void 0===e.buttons?{buttons:Ht(e).length?Ht(e):Ot}:{}}}return{...i,...t}}class Nt extends at{constructor(){super(...arguments),this.users=[],this.addButton=()=>{const t=this.config,e=[...t.buttons??[]],i=Ot[e.length];i&&(this.config={...t,buttons:[...e,i]},this.emitConfigChanged())},this.computeLabel=t=>this.label(t.name)}setConfig(t){this.config=qt(t,this.kind)}willUpdate(t){"overview"===this.kind&&t.has("hass")&&this.loadUsers()}loadUsers(){const t=this.hass?.connection;t&&t!==this.usersConnection&&(this.usersConnection=t,t.sendMessagePromise({type:"config/auth/list"}).then(t=>{this.users=t.filter(t=>t.is_active&&!t.system_generated).sort((t,e)=>t.name.localeCompare(e.name))}).catch(()=>{this.users=[]}))}render(){return this.config?V`
      <ha-form
        .hass=${this.hass}
        .data=${this.formData()}
        .schema=${this.schema()}
  ...
  `}}t([pt({attribute:!1})],Nt.prototype,"hass",void 0),t([ut()],Nt.prototype,"config",void 0),t([ut()],Nt.prototype,"users",void 0);let Bt=class extends Nt{constructor(){super(...arguments),this.kind="daily"}};Bt=t([lt("chores-manager-daily-card-editor")],Bt);let Vt=class extends Nt{constructor(){super(...arguments),this.kind="overview"}};Vt=t([lt("chores-manager-overview-card-editor")],Vt);let Wt=class extends Nt{constructor(){super(...arguments),this.kind="correction"}};Wt=t([lt("chores-manager-correction-card-editor")],Wt);let Lt=class extends Nt{constructor(){super(...arguments),this.kind="history"}};Lt=t([lt("chores-manager-history-card-editor")],Lt);let Ft=class extends mt{constructor(){super(...arguments),this.loadFailed=!1}static getConfigElement(){return document.createElement("chores-manager-history-card-editor")}static getStubConfig(t){return{child_id:t?xt(t)[0]?.id??"kid_1":"kid_1",locale:"auto",show_border:!0,show_header:!0,show_person:!0,show_points:!0}}setConfig(t){if(!t?.child_id?.trim())throw new Error("child_id is required");this.config={locale:"auto",show_border:!0,show_header:!0,show_person:!0,show_points:!0,...t},this.load(),this.requestUpdate()}willUpdate(){this.load()}getCardSize(){return Math.max(2,1+2*this.groupedCompletions().size)}render(){if(!this.hass||!this.config)return L;const t=Et(this.hass,this.config.person_entity),e=this.config.name??zt("weekly_chores",this.config.locale,this.hass);return V`
      <ha-card class=${!1===this.config.show_border?"borderless":""}>
        ${!1!==this.config.show_header?V`
              <header>
            ...
            `:L}
        ${this.loadFailed?V`<p class="error" role="alert">${zt("history_error",this.config.locale,this.hass)}</p>`:this.history?this.renderHistory():L}
      </ha-card>
    `}renderHistory(){const t=this.groupedCompletions();return t.size?V`
      <div class="history">
        ${[...t.entries()].map(([t,e])=>this.renderDay(t,e))}
      </div>
    `:V`<p class="empty">${zt("history_empty",this.config?.locale,this.hass)}</p>`}renderDay(t,e){const i=e.reduce((t,e)=>t+e.points,0);return V`
      <section data-local-date=${t}>
        ...
        ${!1!==this.config?.show_points?V`<strong class="total">${zt("total",this.config?.locale,this.hass)}: ${i}p</strong>`:L}
      </section>
    `}groupedCompletions(){const t=new Map;for(const e of this.history?.completions??[]){const i=t.get(e.local_date)??[];i.push(e),t.set(e.local_date,i)}for(const e of t.values())e.sort((t,e)=>t.category.localeCompare(e.category)||t.chore_title.localeCompare(e.chore_title)||t.completion_id.localeCompare(e.completion_id));return new Map([...t.entries()].sort(([t],[e])=>t.localeCompare(e)))}weekday(t){const e=Mt(this.config?.locale,this.hass);return new Intl.DateTimeFormat("sv"===e?"sv-SE":"en-US",{weekday:"long"}).format(new Date(`${t}T12:00:00`))}load(){const t=this.hass?.connection,e=this.config?.child_id;if(!t||!e)return;const i=function(t,e,i){const s=$t(t,e,i);if(!s)return;const o=bt(s,"week_start");return[s.state,"string"==typeof o?o:"",s.last_updated??""].join("|")}(this.hass,e,this.config?.weekly_points_entity);e===this.requestChildId&&t===this.requestConnection&&i===this.requestUpdateKey||(this.requestChildId=e,this.requestConnection=t,this.requestUpdateKey=i,this.history=void 0,this.loadFailed=!1,t.sendMessagePromise({type:"chores_manager/current_week_history",child_id:e}).then(s=>{this.requestChildId===e&&this.requestConnection===t&&this.requestUpdateKey===i&&(this.history=s)}).catch(()=>{this.requestChildId===e&&this.requestConnection===t&&this.requestUpdateKey===i&&(this.loadFailed=!0)}))}static{this.styles=r`
    :host { display: block; }
  ...
  `}};var Kt,Gt;t([ut()],Ft.prototype,"history",void 0),t([ut()],Ft.prototype,"loadFailed",void 0),Ft=t([lt(yt)],Ft),function(t){t.language="language",t.system="system",t.comma_decimal="comma_decimal",t.decimal_comma="decimal_comma",t.space_comma="space_comma",t.none="none"}(Kt||(Kt={})),function(t){t.language="language",t.system="system",t.am_pm="12",t.twenty_four="24"}(Gt||(Gt={}));const Jt=["closed","locked","off"],Zt=(t,e,i,s)=>{s=s||{},i=null==i?{}:i;const o=new Event(e,{bubbles:void 0===s.bubbles||s.bubbles,cancelable:Boolean(s.cancelable),composed:void 0===s.composed||s.composed});return o.detail=i,t.dispatchEvent(o),o},Qt=t=>{Zt(window,"haptic",t)},Xt=(t,e,i,s,o)=>{let n;if(o&&i.double_tap_action?n=i.double_tap_action:s&&i.hold_action?n=i.hold_action:!s&&i.tap_action&&(n=i.tap_action),n||(n={action:"more-info"}),!n.confirmation||n.confirmation.exemptions&&n.confirmation.exemptions.some(t=>t.user===e.user.id)||confirm(n.confirmation.text||`Are you sure you want to ${n.action}?`))switch(n.action){case"more-info":(n.entity||i.entity||i.camera_image)&&(Zt(t,"hass-more-info",{entityId:n.entity?n.entity:i.entity?i.entity:i.camera_image}),n.haptic&&Qt(n.haptic));break;case"navigate":n.navigation_path&&(((t,e,i=!1)=>{i?history.replaceState(null,"",e):history.pushState(null,"",e),Zt(window,"location-changed",{replace:i})})(0,n.navigation_path),n.haptic&&Qt(n.haptic));break;case"url":n.url_path&&window.open(n.url_path),n.haptic&&Qt(n.haptic);break;case"toggle":i.entity&&(((t,e)=>{((t,e,i=!0)=>{const s=function(t){return t.substr(0,t.indexOf("."))}(e),o="group"===s?"homeassistant":s;let n;switch(s){case"lock":n=i?"unlock":"lock";break;case"cover":n=i?"open_cover":"close_cover";break;default:n=i?"turn_on":"turn_off"}t.callService(o,n,{entity_id:e})})(t,e,Jt.includes(t.states[e].state))})(e,i.entity),n.haptic&&Qt(n.haptic));break;case"call-service":{if(!n.service)return;const[t,s]=n.service.split(".",2),o=Object.assign({},n.service_data);"entity"===o.entity_id&&(o.entity_id=i.entity),e.callService(t,s,o,n.target),n.haptic&&Qt(n.haptic);break}case"fire-dom-event":Zt(t,"ll-custom",n),n.haptic&&Qt(n.haptic)}},Yt={amber:"#ffc107",blue:"#2196f3",cyan:"#00bcd4",green:"#4caf50",orange:"#ff9800",purple:"#9c27b0",red:"#f44336",teal:"#009688",yellow:"#ffeb3b"},te=/^#[0-9a-f]{6}$/iu,ee=[{label:"Chores",icon:"mdi:format-list-checks",color:"#00bcd4"},{label:"History",icon:"mdi:trophy-outline",color:"#ffc107"},{label:"Correction",icon:"mdi:wrench-cog",color:"#9c27b0"}];let ie=class extends mt{constructor(){super(...arguments),this.weeklyPointsError=!1,this.adjustmentPending=!1,this.adjustmentError=!1,this.heldButtons=new WeakSet,this.holdTimers=new WeakMap,this.clickTimers=new WeakMap}static getConfigElement(){return document.createElement("chores-manager-overview-card-editor")}static getStubConfig(){return{child_id:"kid_1",goal_points:20,progress_color:"#00a6d6",person_position:"center",locale:"auto",rewards:[{points:20,label:"Weekly reward",color:"#34c759"},{points:30,label:"Weekly reward and allowance",color:"#ff9f0a"}]}}setConfig(t){if(!t?.child_id?.trim()&&!t?.child_entity?.trim())throw new Error("child_id or child_entity is required");if(void 0!==t.goal_points&&t.goal_points<=0)throw new Error("goal_points must be above zero");if(this.config={locale:"auto",person_position:"center",person_size:"medium",show_name:!0,show_person:!0,show_points:!0,show_previous_week:!0,show_adjustments:!0,show_border:!0,rewards:[],...t},this.hass){const t=kt(this.hass,this.config);t&&this.loadWeeklyPoints(t)}this.requestUpdate()}willUpdate(t){if(!t.has("hass"))return;const e=this.hass&&this.config?kt(this.hass,this.config):void 0;if(e){Pt(this.hass,e,this.config?.weekly_points_entity??this.config?.child_entity)===this.confirmedPoints&&(this.confirmedPoints=void 0),this.loadWeeklyPoints(e)}}getCardSize(){return 5}render(){if(!this.hass||!this.config)return L;const t=kt(this.hass,this.config);if(!t)return L;const e=Pt(this.hass,t,this.config.weekly_points_entity??this.config.child_entity)??0,i=this.confirmedPoints??e,s=At(this.hass,t),o=[...this.config.rewards??[]].sort((t,e)=>t.points-e.points),n=this.nextReward(i,o),r=n?.points??o.at(-1)?.points??this.config.goal_points??20,a=Et(this.hass,this.config.person_entity),c=this.config.name??Ct(this.hass,t)??t,l=this.config.person_position??"left",h=this.config.person_size??"medium",d=Math.min(100,Math.round(i/r*100)),p=this.progressColor(i,o),u=this.buttons().filter(t=>this.isVisible(t));return V`
      <ha-card class=${!1===this.config.show_border?"borderless":""}>
              ...
              <header>
                <div class="overview-heading">
                  ...
                  ${!1!==this.config.show_previous_week&&this.weeklyPoints?V`<span class="previous-week">
                        ${zt("previous_week",this.config.locale,this.hass)} ·
                        ${this.weeklyPoints.previous_week.points}
                      </span>`:L}
                </div>
                ${!1!==this.config.show_person?V`<div class="portrait-row position-${l}">
                      ${a?V`<img class="portrait size-${h}" src=${a} alt="" />`:V`<ha-icon class="portrait-icon size-${h}" icon="mdi:account-circle"></ha-icon>`}
                    </div>`:L}
              </header>
            `:L}
        <div class="points-row" ?hidden=${!1===this.config.show_points}>
          ...
          <div>
            <strong>${i} / ${r} ${zt("points",this.config.locale,this.hass)}</strong>
    ...
    `}renderCompactAdjustment(t){const e=!1!==this.config?.show_adjustments;if(this.weeklyPointsError)return V`
        <p class="api-error" role="alert">
          ${zt("weekly_points_error",this.config?.locale,this.hass)}
        </p>
    ...
    `}loadWeeklyPoints(t){const e=this.hass?.connection;if(!e)return;const i=St(this.hass,t,this.config?.weekly_points_entity??this.config?.child_entity);t===this.weeklyPointsChildId&&e===this.weeklyPointsConnection&&i===this.weeklyPointsWeekStart||(this.weeklyPointsChildId=t,this.weeklyPointsConnection=e,this.weeklyPointsWeekStart=i,this.weeklyPoints=void 0,this.weeklyPointsError=!1,e.sendMessagePromise({type:"chores_manager/weekly_points",child_id:t}).then(s=>{this.weeklyPointsChildId===t&&this.weeklyPointsConnection===e&&this.weeklyPointsWeekStart===i&&(this.weeklyPoints=s)}).catch(()=>{this.weeklyPointsChildId===t&&this.weeklyPointsConnection===e&&this.weeklyPointsWeekStart===i&&(this.weeklyPointsError=!0)}))}async adjustPoints(t,e){const i=this.hass?.connection;if(i&&!this.adjustmentPending){this.adjustmentPending=!0,this.adjustmentError=!1;try{const s=await i.sendMessagePromise({type:"chores_manager/adjust_weekly_points",child_id:t,amount:e});this.confirmedPoints=s.current_points,this.weeklyPoints?.child_id===t&&(this.weeklyPoints={...this.weeklyPoints,current_week:{...this.weeklyPoints.current_week,points:s.current_points}})}catch{this.adjustmentError=!0}finally{this.adjustmentPending=!1}}}renderPointsAndRewards(t,e){const i=new Map;for(const e of t){const t=i.get(e.points)??[];t.push(e),i.set(e.points,t)}return V`
      <details class="points-rewards">
        <summary>
          <span>${zt("rewards",this.config?.locale,this.hass)}</span>
          <p>${zt("how_points_work",this.config?.locale,this.hass)}</p>
        </summary>
        <div class="rewards-content">
          ${i.size?V`
                <section>
                  <h2>${zt("chores",this.config?.locale,this.hass)}</h2>
                  ${[...i.entries()].sort(([t],[e])=>e-t).map(([t,e])=>V`
                        <h3>${t} ${zt("points",this.config?.locale,this.hass)}</h3>
          ...
          ${e.length?V`
                <section>
                  <h2>${zt("reward_levels",this.config?.locale,this.hass)}</h2>
